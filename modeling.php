<!DOCTYPE html>

<html lang="en-US">

<head>

<title>Jake Schweihs - Technical Artist</title>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta name="description" content="Jake Schweihs' 3d rigging and modeling portfolio" />
<meta name="keywords" content="rigging, modeling, maya, technical artist, 3d art" />
<meta name="robots" content="all" />
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1, user-scalable=no">

<link rel="canonical" href="/modeling" />
<link rel="stylesheet" type="text/css" href="style.css">

</head>

<body>

<?php include("nav.php"); ?>

<div id="contentFrame">
	<div id="contentTop"></div>
	<div id="contentMid">
		<div id="innerContentNews">
			<br>
			<div id="pixelsBlock">
			<h1>Modeling</h1>
			<br>
			<br><h2>Still Life</h2><br>
			<a href="/images/starWarsStillLife.png"><img width="720" height="405" src="/images/starWarsStillLife.png" alt="Star Wars Still Life"></a>
			<p class="captionP">A still life from a galaxy far, far away.</p>
			<br><h2>Dota 2</h2><br>
			<a href="/images/dragonKnightSkin.png"><img width="720" height="420" src="/images/dragonKnightSkin.png" alt="Dragon Knight Skin"></a>
			<p class="captionP">Custom skin for Dragon Knight from Dota 2.</p>
			<br><h2>Interiors</h2><br>
			<a href="/images/bedoomInterior.jpg"><img width="720" height="405" src="/images/bedoomInterior.jpg" alt="Bedroom Interior"></a>
			<p class="captionP">Bedroom Night Interior in Mental Ray</p>
			<br><h2>Blacksmith</h2><br>
			<img class="transAsset" src="/images/castanerTurnaround.gif" alt="Blacksmith Turnaround">
			<p class="captionP">Modeled in zBrush and rendered in Mental Ray</p>
			<img src="/images/castanerHighres.png" alt="Blacksmith Close Up">
			<p class="captionP">zBrush Screenshot</p>

			</div>
		</div>
	</div>

	<div id="contentBot"></div>


</div>

<?php include("footer.php"); ?>

</body>

</html>
